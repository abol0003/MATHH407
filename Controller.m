...
N_lqi = 1.251103617063341;
K_lqi = [-46.927264507682290,-5.692017919559630];
...
r_desired = ones(N0,1) * 0.11; %meters

time = 0:Ts:(N0-1)*Ts; %Vector saving the time steps.
% Fsin = 2;%peak at 2.2 rad/s(bode sim)
% Ampsin = 0.01;
% r_desired = Ampsin * sin(time * Fsin) + 0.108; %meters
...
while cond == 1
    ...
    %lqr controller
    integration = integration + r_error  * Ts;
...
figure
subplot(4, 1, 1);
plot(time(:), Data(:,3), time(:), r_desired(:), time(:), Data(:, 5));
legend("r(m)", "r_d(m)", "int")
ylabel("r(m)");

subplot(4, 1, 2);
plot(time(:), Data(:,1), time(:), Data(:,3) * 100);
legend("r(V)", "r(cm)")
ylabel("r(V) & r(cm)");

subplot(4, 1, 3);
plot(time(:), Data(:,2), time(:), Data(:,6));
legend("w_I(V)", "w_O(V)")
ylabel("w_I(V) & w_O(V)");
